...
To return this root's max path, we compare the max of these path directions:
1. root only
1. root + left subtree
2. root + right subtree
^ We MUST use the root node as that is the anchor of the current recursive call.
If you don't include root, you'll always return a 0 sum which is invalid.
"""
class Solution:
    def maxPathSum(self, root: Optional[TreeNode]) -> int:
        ...
        def helper(root):
            ...
            # Return the largest path: root, root + left, root + right
            # Note: Cannot return root + left + right because that's an invalid path recursing up the tree 
