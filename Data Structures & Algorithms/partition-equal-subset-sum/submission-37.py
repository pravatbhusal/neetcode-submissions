        ...
        for num in nums:
            ...
            for cur_sum in dp:
                if (cur_sum + num) == target:
                    return True
                next_dp.add(cur_sum + num)
            dp.update(next_dp)

        return False

    # Top-down recursive solution has same time complexity as iterative but O(N * total) space
    def canPartition_Recursive_Top_Down(self, nums: List[int]) -> bool:
        total = sum(nums)
        if total % 2 == 1:
            # short-circuit, odd sum cannot be partitioned
            return False
