class Solution:
    ...
    find() - returns the "root" node of a connected component
    union() - connects two roots under the same root, with the highest "rank"
    parent - list of parent node for each node, find() updates this to to the root node (path compression)
    rank - list of height for each node
    """
    def countComponents(self, n: int, edges: List[List[int]]) -> int:
        ...
        # run union on every edge, remainder is the number of disjoint sets
        disjoints = n
        for n1, n2 in edges:
    ...
    """ 
    Note: Keeping this BFS solution because it is a valid.
    The more optimal solution is Union-Find, see above solution.
