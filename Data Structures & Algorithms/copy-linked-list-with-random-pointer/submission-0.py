...
class Solution:
    def copyRandomList(self, head: 'Optional[Node]') -> 'Optional[Node]':
        ...
        # fill copies with original refs for the deep copied node
        cur = head
        while cur:
            copy = Node(cur.val, copies.get(cur.next), copies.get(cur.random))
            copies[cur] = copy
            cur = cur.next

        # second pass now that all refs are stored
        cur = head
        while cur:
