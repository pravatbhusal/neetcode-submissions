class Solution:
    ...
    def leastInterval(self, tasks: List[str], n: int) -> int:
        ...
        while max_heap or queue:
            ...
            while queue:
                available_at = queue[0][2]
                if available_at > cycles:
            ...
            if max_heap:
                ...
                # do task
                freq *= -1 # negate (because of max_heap)
                freq -= 1
                cycles += 1

                if freq > 0:
                    # task not complete, add back to queue
                    available_at = cycles + n
                    queue.append((task, freq, available_at))
            else:
                # idle — skip time and jump to next available task
                cycles = queue[0][2]
