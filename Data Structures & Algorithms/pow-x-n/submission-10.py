...
This idea is called "binary exponentiation" (or exponentiation by squaring). Here's an example of binary exponentation,
...
We did not have to calculate 2^3 because binary exponentation skipped that calculation. binary exponentation skips
...
class Solution:
    ...
    This solution is O(log(N)) time and O(1) space.
    """
    ...
    """
    This is a top-down recursive solution using binary exponentiation formula: x^n = (x^2)^(n/2).
    We are building the solution top-down, starting from x^n to the bottom result = 1.
    This solution is O(log(N)) time and space.
    """
    def myPow_TopDown_Recursive(self, x: float, n: int) -> float:
        ...
        def helper(x, n):
            if n == 0:
                return 1
            if n % 2 == 0:
                # handle even (binary exponentation)
