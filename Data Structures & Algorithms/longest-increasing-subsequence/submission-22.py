    ...
    def lengthOfLIS(self, nums: List[int]) -> int:
        ...
        for i in range(n - 1, -1, -1):
            for j in range(i + 1, n):
                if nums[j] > nums[i]:  # verify keeping num makes sub-array stay sorted
                    # recurrence function comparing taking num vs not taking num
                    dp[i] = max(dp[i], 1 + dp[j])

        return max(dp)

    # Recursive solution, same time and space complexity as iterative
