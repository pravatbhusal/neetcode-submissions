...
class Solution:
    ...
    # Top-down Recursive solution, same time complexity as iterative but O(N^2) space
    # The only difference is that Leetcode throws a Recursion Max-depth exception
    def lengthOfLIS_Recursive_Top_Down(self, nums: List[int]) -> int:
        dp = {}
        temp = []
        ...
        def helper(i, prev):
            ...
            # keep this number
            result = 0
            if prev == -1 or nums[i] > nums[prev]: # verify keeping num makes sub-array stay sorted
