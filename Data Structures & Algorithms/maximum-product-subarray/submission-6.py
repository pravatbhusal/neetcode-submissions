...
Can we the concept of Kadane's algorithm to make similar recurrence function for this max product subarray problem?
...
Recurrence function (max case): max_product = max(max_product * nums[i], min_product * nums[i], nums[i])
Recurrence function (min case): min_product = min(max_product * nums[i], min_product * nums[i], nums[i])
"""
class Solution:
    def maxProduct(self, nums: List[int]) -> int:
        ...
        for i in range(1, len(nums)):
            ...
            # Update global_max_product because max_product is local max (Kadane's greedy algorithm)
            global_max_product = max(max_product, global_max_product)
