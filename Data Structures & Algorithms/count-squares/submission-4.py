...
class CountSquares:
    ...
    def count(self, point: List[int]) -> int:
        ...
        for y2, freq in list(self.points[x1].items()):
            ...
            # distance between (x1, y1) and (x1, y2)
            # use equi-distance to calculate coords for the 3 other points
            distance = abs(y1 - y2)

            # point (x1, y2)
            vert_combos = self.points[x1][y2]
            
            # point for left-side (x2, y1)
            x2 = x1 - distance
            left_combos = self.points[x2][y1]
            # point for left-side (x2, y2)
            left_combos *= self.points[x2][y2]
            
            # point for right-side (x2, y1)
            x2 = x1 + distance
            right_combos = self.points[x2][y1]
            # point for right-side (x2, y2)
            right_combos *= self.points[x2][y2]

            # sum total square combinations from this x-axis
