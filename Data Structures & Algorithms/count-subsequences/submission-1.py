...
Because the recurrence ahead pointers (i + 1 and j + 1), the iteration must
happen in reverse.
...
We can further optimize this to 1DP in O(len(t)) space. Since the inner loop
iterates at i for every j, we only need to have a single variable prev where
