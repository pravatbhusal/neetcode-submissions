...
Timeline sweep solved using two ptrs: start_ptr and end_ptr.
1. Create two sorted arrays of the start and end times for each meeting.
2. One ptr will track the start time, another ptr will track the end time.
3. When the start time of a new meeting is less than the earliest end time
seen so far, we need a new room. When a meeting starts after the earliest
end time, a room has freed up so we advance end_ptr to the next earliest
end time.
"""
class Solution:
