...
Max water at i = min(max_left, max_right) - height[i]
...
When we process left-side, max water at i = max_left - height[left]
When we process right-side, max water at i = max_right - height[right]
"""
class Solution:
    def trap(self, height: List[int]) -> int:
        ...
        while left != right:
            if max_left < max_right:
                ...
                left += 1
                # sum the max water at i = left ptr
                max_left = max(height[left], max_left)
                total += max_left - height[left]
            else:
                # keep taller left-side height, so move right ptr
                right -= 1
                # sum the max water at i = right ptr
