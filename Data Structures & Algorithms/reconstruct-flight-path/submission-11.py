...
Then visit every node in the graph and and pop from the end of the adjacency list to get the smallest (because descending order).
Add the node to the result after visiting all neighboring edges (post-order DFS).
The post-order DFS and reversing the result is very similar to DFS Topological sort.
"""
class Solution:
